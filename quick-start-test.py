...
# Save as CSV
pd.DataFrame(emb_0.cpu().numpy()).to_csv('embedding_0.csv', index=False)
pd.DataFrame(emb_1.cpu().numpy()).to_csv('embedding_1.csv', index=False)

# Per-protein embedding (single row)
pd.DataFrame([emb_0_per_protein.cpu().numpy()]).to_csv('embedding_0_per_protein.csv', index=False)
