#!/bin/bash
#$ -cwd
#$ -o /u/scratch/a/aliceg/logs/misc/compare_h100.$JOB_ID.out
#$ -j y
# Equivalence-test REFERENCE run: one full production chunk (pfam_chunk_05)
# searched against the WHOLE 29G padded target DB on an H100 (96G VRAM, fits
# whole). This is the ground truth the sharded RTX run must reproduce.
#$ -l gpu,H100,cuda=1,h_rt=8:00:00,h_data=48G
#$ -M $USER@ucla.edu
#$ -m bea
...
TASK_OUTPUT="$SCRATCH/rtx_vs_h200_test/h100_chunk05"
